    ...
    if (!selectedLayer) {
       return (
         <div className="h-[200px] flex flex-col items-center justify-center border border-dashed border-zinc-300 dark:border-white/10 rounded-3xl text-zinc-500 text-center px-6 mt-4">
           <Layers size={32} className="text-zinc-400 mb-3" />
    ...
    <div className="flex flex-col h-full select-none bg-[#f4f4f5] dark:bg-[#050505] text-zinc-900 dark:text-zinc-100 relative transition-colors duration-300">
      ...
      <div className="flex justify-between items-center px-5 py-3 border-b border-zinc-200 dark:border-white/5 bg-white/50 dark:bg-black/20 shrink-0">
        ...
        <div className="flex gap-2">
          <button onClick={undo} disabled={past.length === 0} className={`p-1.5 rounded-lg transition-all ${past.length === 0 ? 'opacity-30' : 'hover:bg-zinc-200 dark:hover:bg-zinc-800 active:scale-90'}`}><Undo2 size={15} /></button>
          <div className="w-px bg-zinc-300 dark:bg-white/10 my-1"></div>
          <button onClick={redo} disabled={future.length === 0} className={`p-1.5 rounded-lg transition-all ${future.length === 0 ? 'opacity-30' : 'hover:bg-zinc-200 dark:hover:bg-zinc-800 active:scale-90'}`}><Redo2 size={15} /></button>
        </div>
      </div>
      ...
      <div className="flex gap-2 p-3 bg-white/80 dark:bg-black/80 backdrop-blur-3xl border-t border-zinc-200 dark:border-white/10 shrink-0 z-20 pb-6 md:pb-4">
        {[
          ...
          { id: 'edit', icon: <Type size={20} />, label: 'Edit Tools' },
          { id: 'effects', icon: <Sparkles size={20} />, label: 'Effects' }
        ].map((tab) => (
          <button 
            key={tab.id}
            onClick={() => setActiveTab(tab.id as any)} 
            className={`flex-1 flex flex-col justify-center items-center gap-1.5 py-3 rounded-2xl transition-all duration-300 ${activeTab === tab.id ? 'bg-blue-500 text-white shadow-lg' : 'hover:bg-zinc-200 dark:hover:bg-white/5 text-zinc-500'}`}
          >
            {tab.icon}
